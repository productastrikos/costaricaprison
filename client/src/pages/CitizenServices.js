...
export default function Services() {
    ...
    return (<div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-app-panel border border-app-border rounded-xl overflow-hidden">
          <div className="flex border-b border-app-border overflow-x-auto">
            {TABS.map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className="px-4 py-3 text-xs font-semibold whitespace-nowrap transition-colors shrink-0" style={{
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-app-border">
                  {['ID', 'Subject', 'Category', 'Status', 'Priority', 'User', 'Created'].map(h => (<th key={h} className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'var(--app-text-faint)' }}>{h}</th>))}
                </tr>
              </thead>
              <tbody>
                {filtered.map(req => {
            ...
            return (<tr key={req.id} className="border-b border-app-border/50 hover:bg-white/[0.02] transition-colors cursor-pointer" onClick={() => setSelectedReq(req)}>
                ...
                {filtered.length === 0 && (<tr><td colSpan={7} className="px-3 py-8 text-center text-xs" style={{ color: 'var(--app-text-faint)' }}>No requests in this category.</td></tr>)}
              </tbody>
            </table>
          </div>
        </div>
      ...
      {selectedReq && (<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setSelectedReq(null)}>
          <div className="bg-app-panel border border-app-border rounded-xl p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            ].map(({ label, value }) => (<div key={label} className="bg-white/[0.03] rounded-lg p-3">
            ...
            <div className="flex gap-2">
              <button className="flex-1 py-2 rounded-lg text-xs font-semibold transition-all" style={{ background: 'var(--app-accent-bg)', border: '1px solid var(--app-accent-border)', color: 'var(--app-accent)' }} onClick={() => setSelectedReq(null)}>Mark Resolved</button>
              <button className="flex-1 py-2 rounded-lg text-xs font-semibold transition-all" style={{ background: 'transparent', border: '1px solid var(--app-border)', color: 'var(--app-text-faint)' }} onClick={() => setSelectedReq(null)}>Reassign</button>
            </div>
          </div>
        </div>)}
    </div>);
