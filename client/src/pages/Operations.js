...
import KPICard, { IcoCoverage, IcoCheck, IcoAlert, IcoCalendar, IcoTruck, IcoWrench, IcoBox, IcoDollar, IcoTrendUp, } from '../components/KPICard';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-app-panel border border-app-border rounded-xl overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-app-border">
                  {['ID', 'Name', 'Category', 'Status', 'Priority', 'Assignee', 'Updated'].map(h => (<th key={h} className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'var(--app-text-faint)' }}>{h}</th>))}
                </tr>
              </thead>
              <tbody>
                {filtered.map(rec => {
            ...
            return (<tr key={rec.id} className="border-b border-app-border/50 hover:bg-white/[0.02] transition-colors">
                ...
                {filtered.length === 0 && (<tr><td colSpan={7} className="px-3 py-8 text-center text-xs" style={{ color: 'var(--app-text-faint)' }}>No records match the current filter.</td></tr>)}
              </tbody>
            </table>
          </div>
        </div>
      </div>
      {selectedKPI && (<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setSelectedKPI(null)}>
...
function AssetsTab() {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-app-panel border border-app-border rounded-xl overflow-hidden">
          <div className="flex border-b border-app-border">
            {['Overview', 'Maintenance', 'Assignments'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className="px-4 py-3 text-xs font-semibold transition-colors" style={{ color: activeTab === tab ? 'var(--app-accent)' : 'var(--app-text-faint)', borderBottom: activeTab === tab ? '2px solid var(--app-accent)' : '2px solid transparent' }}>
                {tab}
              </button>))}
          </div>
          {activeTab === 'Overview' ? (<div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-app-border">
                    {['ID', 'Name', 'Type', 'Status', 'Location', 'Utilization', 'Last Seen'].map(h => (<th key={h} className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'var(--app-text-faint)' }}>{h}</th>))}
                  </tr>
                </thead>
                <tbody>
                  {ASSETS.map(asset => {
                const sm = AST_STATUS[asset.status];
                return (<tr key={asset.id} className="border-b border-app-border/50 hover:bg-white/[0.02] transition-colors cursor-pointer" onClick={() => setSelectedAsset(asset)}>
                        ...
                        <td className="px-3 py-2.5">
                          {asset.utilization > 0 ? (<div className="flex items-center gap-2">
                            ...
                            </div>) : <span style={{ color: 'var(--app-text-faint)' }}>—</span>}
                        </td>
      ...
      {selectedAsset && (<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setSelectedAsset(null)}>
          <div className="bg-app-panel border border-app-border rounded-xl p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              {[{ label: 'Type', value: selectedAsset.type }, { label: 'Status', value: AST_STATUS[selectedAsset.status]?.label }, { label: 'Location', value: selectedAsset.location }, { label: 'Utilization', value: selectedAsset.utilization > 0 ? `${selectedAsset.utilization}%` : 'N/A' }, { label: 'Last Seen', value: selectedAsset.lastSeen }].map(({ label, value }) => (<div key={label} className="bg-white/[0.03] rounded-lg p-3">
...
function InventoryTab() {
    ...
    const sorted = [...INVENTORY]
        ...
        if (sortBy === 'used')
            return (b.used / b.capacity) - (a.used / a.capacity);
        if (sortBy === 'daily')
            return b.dailyIn - a.dailyIn;
        return 0;
    });
    return (<div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          {['all', 'normal', 'warning', 'critical'].map(s => (<button key={s} onClick={() => setFilter(s)} className="px-3 py-1 rounded text-xs font-semibold transition-all capitalize" style={{ background: filterStatus === s ? 'var(--app-accent-bg)' : 'transparent', border: `1px solid ${filterStatus === s ? 'var(--app-accent-border)' : 'var(--app-border)'}`, color: filterStatus === s ? 'var(--app-accent)' : 'var(--app-text-faint)' }}>
              {s}
            </button>))}
        </div>
        <div className="flex items-center gap-2 ml-auto">
          <span className="text-xs" style={{ color: 'var(--app-text-faint)' }}>Sort:</span>
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="text-xs rounded px-2 py-1 outline-none" style={{ background: 'var(--app-surface)', border: '1px solid var(--app-border)', color: 'var(--app-text)' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {sorted.map(inv => {
            ...
            return (<div key={inv.id} className="bg-app-panel border border-app-border rounded-xl p-4">
    ...
    return (<div className="p-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-1 bg-app-panel border border-app-border rounded-lg p-1">
          {TABS.map(t => (<button key={t} onClick={() => setTab(t)} className="px-4 py-1.5 rounded text-xs font-semibold transition-all" style={{ background: tab === t ? 'var(--app-accent-bg)' : 'transparent', color: tab === t ? 'var(--app-accent)' : 'var(--app-text-faint)', border: `1px solid ${tab === t ? 'var(--app-accent-border)' : 'transparent'}` }}>
              {t}
            </button>))}
        </div>
      </div>
