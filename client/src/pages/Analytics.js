...
import KPICard, { IcoLeaf, IcoTrendUp, IcoShield, IcoGlobe, IcoBarChart, IcoCheck, IcoHourglass, IcoBolt, IcoSignal, IcoThermometer, } from '../components/KPICard';
...
function makeSeries(base, variance, length = 24) {
    return Array.from({ length }, (_, i) => +(base + (Math.sin(i * 0.4) * variance + (Math.random() - 0.5) * variance * 0.5)).toFixed(1));
}
// ── Tabs ──────────────────────────────────────────────────────────────────────
      ...
      <div className="bg-app-panel border border-app-border rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-app-border">
          {INITIATIVES.map(init => {
            ...
            return (<div key={init.name} className="flex items-center gap-4 px-4 py-3">
...
function WorkflowTab() {
    ...
    return (<div className="space-y-6">
      ...
      {view === 'pipeline' && (<div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {STAGES.map((stage, idx) => {
                const sm = STAGE_META[stage.status];
                const pct = totalIn > 0 ? (stage.items / totalIn) * 100 : 0;
                return (<div key={stage.id} className="bg-app-panel border border-app-border rounded-xl p-4 relative">
                {idx < STAGES.length - 1 && (<div className="hidden lg:block absolute -right-4 top-1/2 -translate-y-1/2 z-10 text-slate-600 text-lg">›</div>)}
      ...
      {view === 'chart' && (<div className="bg-app-panel border border-app-border rounded-lg p-3 relative">
          <div className="flex items-center gap-1 mb-0.5">
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {UNITS.map(unit => {
            ...
            return (<div key={unit.id} className="bg-app-panel border border-app-border rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
