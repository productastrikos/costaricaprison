      ...
      <div className="h-full overflow-y-auto p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {(() => {
            ...
            return (<KPICard icon={<IcoCoverage />} label="Tasa de Rendimiento" value={`${(kz.collectionCoverage || 96.2).toFixed(1)}%`} trend={kz.coverageTrend || 1.1} color={col} subValues={[{ label: 'Exceptions', value: `${(kz.missedCollections || 1.8).toFixed(1)}%` }, { label: segmentFilter === 'all' ? 'Segments' : 'Groups', value: segmentFilter === 'all' ? `${kz.zonesServed || 5}/5` : `${kz.groupCount || 3}` }]} onClick={() => showKPIDetail({ icon: <IcoCoverage />, label: 'Tasa de Rendimiento', value: (kz.collectionCoverage || 96.2).toFixed(1), unit: '%', trend: kz.coverageTrend || 1.1, color: col, thresholds: { green: 95, amber: 85 }, inverted: false, definition: 'Percentage of scheduled operations completed successfully. Replace this definition with a description of your own metric.', subValues: [{ label: 'Units Processed', value: kz.rfidScans || '4,430' }, { label: 'Exceptions', value: kz.missedPoints || 83 }, { label: 'At-Risk Groups', value: 1 }, { label: 'Target', value: '≥95% daily' }], target: '95%+ daily performance rate' })}/>);
        })()}
          {(() => {
            const col = (kz.missedCollections || 1.8) <= 2 ? 'text-emerald-400' : (kz.missedCollections || 1.8) <= 5 ? 'text-amber-400' : 'text-red-400';
            return (<KPICard icon={<IcoAlert />} label="Tasa de Excepciones" value={`${(kz.missedCollections || 1.8).toFixed(1)}%`} trend={-(kz.missedTrend || -0.6)} color={col} subValues={[{ label: 'Count', value: kz.missedPoints || 83 }, { label: 'Alerts', value: kz.overdueAlerts || 2 }]} onClick={() => showKPIDetail({ icon: <IcoAlert />, label: 'Tasa de Excepciones', value: (kz.missedCollections || 1.8).toFixed(1), unit: '%', trend: -(kz.missedTrend || -0.6), color: col, thresholds: { green: 2, amber: 5 }, inverted: true, definition: 'Percentage of operations resulting in exceptions. Replace with your own exception metric definition.', subValues: [{ label: 'Open Issues', value: kz.missedPoints || 83 }, { label: 'Active Alerts', value: kz.overdueAlerts || 2 }, { label: 'Avg Delay', value: '18 min' }, { label: 'Auto-Assigned', value: '3 tasks' }], target: '<2% exception rate' })}/>);
        })()}
          {(() => {
            const col = (k.routeSavings || 22.4) >= 20 ? 'text-emerald-400' : 'text-amber-400';
            return (<KPICard icon={<IcoRoute />} label="Ahorro de Eficiencia" value={`${(k.routeSavings || 22.4).toFixed(1)}%`} trend={k.routeTrend || 2.3} color={col} subValues={[{ label: 'Cost Saved', value: `$${(k.fuelSaved || 54).toFixed(0)}k` }, { label: 'Time Saved', value: `${k.kmSaved || 386} hrs` }]} onClick={() => showKPIDetail({ icon: <IcoRoute />, label: 'Ahorro de Eficiencia', value: (k.routeSavings || 22.4).toFixed(1), unit: '%', trend: k.routeTrend || 2.3, color: col, thresholds: { green: 20, amber: 10 }, inverted: false, definition: 'Reduction in operational cost vs unoptimised baseline. Replace with your own efficiency metric.', subValues: [{ label: 'Cost Saved', value: `$${(k.fuelSaved || 54).toFixed(0)}k` }, { label: 'Time Saved', value: `${k.kmSaved || 386} hrs` }, { label: 'Optimised Units', value: 21 }, { label: 'Emissions Avoided', value: '412 kg CO₂' }], target: '20%+ cost reduction' })}/>);
        })()}
          {(() => {
            const col = (kz.collectionRate || 96.2) >= 95 ? 'text-emerald-400' : (kz.collectionRate || 96.2) >= 80 ? 'text-amber-400' : 'text-red-400';
            return (<KPICard icon={<IcoTrash />} label="Volumen Diario" value={(kz.dailyCollectionTons ?? 1325).toFixed(0)} unit=" units/day" trend={kz.collectionTrend || 3.2} color={col} subValues={[{ label: 'Target', value: '1,400' }, { label: 'Rate', value: `${(kz.collectionRate || 96.2).toFixed(0)}%` }]} onClick={() => showKPIDetail({ icon: <IcoTrash />, label: 'Volumen Diario', value: (kz.dailyCollectionTons ?? 1325).toFixed(0), unit: 'units/day', trend: kz.collectionTrend || 3.2, color: col, thresholds: { green: 1200, amber: 900 }, inverted: false, definition: 'Total units processed daily. Replace with your own volumetric metric.', subValues: [{ label: 'Target', value: '1,400/day' }, { label: 'Rate', value: `${(kz.collectionRate || 96.2).toFixed(0)}%` }, { label: 'Confirmations', value: '142' }, { label: 'Avg/Resource', value: '28.4' }], target: '1,400 units/day' })}/>);
        })()}
          {(() => {
            const col = (k.recyclingRate || 31.4) >= 30 ? 'text-emerald-400' : 'text-amber-400';
            return (<KPICard icon={<IcoRecycle />} label="Puntaje de Calidad" value={`${(k.recyclingRate || 31.4).toFixed(1)}%`} trend={k.recyclingTrend || 1.8} color={col} subValues={[{ label: 'Passed', value: '416 units' }, { label: 'Value', value: '$3.2k' }]} onClick={() => showKPIDetail({ icon: <IcoRecycle />, label: 'Puntaje de Calidad', value: (k.recyclingRate || 31.4).toFixed(1), unit: '%', trend: k.recyclingTrend || 1.8, color: col, thresholds: { green: 30, amber: 15 }, inverted: false, definition: 'Percentage of processed units meeting quality criteria. Replace with your own quality metric.', subValues: [{ label: 'Units Passed', value: '416' }, { label: 'Redirected', value: '112 units' }, { label: 'Value', value: '$3.2k' }, { label: 'Diverted', value: '528 units' }], target: '30%+ quality rate' })}/>);
        })()}
          {(() => {
            const col = (bins.overflowPct || 0.7) <= 2 ? 'text-emerald-400' : 'text-red-400';
            return (<KPICard icon={<IcoBarChart />} label="Incumplimientos de Umbral" value={`${(bins.overflowPct || 0.7).toFixed(1)}%`} trend={-(k.overflowTrend || -0.3)} color={col} subValues={[{ label: 'Units', value: `${bins.overflow || 3}/${bins.total || 445}` }, { label: 'Near-Threshold', value: bins.needsCollection || 22 }]} onClick={() => showKPIDetail({ icon: <IcoBarChart />, label: 'Incumplimientos de Umbral', value: (bins.overflowPct || 0.7).toFixed(1), unit: '%', trend: -(k.overflowTrend || -0.3), color: col, thresholds: { green: 2, amber: 5 }, inverted: true, definition: 'Percentage of monitored units above configured threshold. Target <2%. Replace with your own threshold metric.', subValues: [{ label: 'Breaching', value: `${bins.overflow || 3}` }, { label: 'Total Monitored', value: bins.total || 445 }, { label: 'Near-Threshold', value: bins.needsCollection || 22 }, { label: 'Sensor Uptime', value: '99.4%' }], target: '<2% threshold breaches' })}/>);
        })()}
        </div>
