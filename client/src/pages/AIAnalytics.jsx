...
export default function AIAnalytics() {
    ...
    const fire = (rec) => {
        ...
        // Remove the executed recommendation card from the panel (keep the toast).
        setExecutedIds((prev) => new Set(prev).add(rec.id));
    };
    const visibleRecs = useMemo(() => c.recommendations.filter((r) => !executedIds.has(r.id)), [c.recommendations, executedIds]);
    const critRecs = useMemo(() => visibleRecs.filter((r) => r.priority === 'critical' || r.priority === 'high'), [visibleRecs]);
    ...
    return (<div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        ...
        <Panel title={t('ai.recsTitle')} icon={<Icon name="ai" className="w-4 h-4"/>} subtitle={t('ai.recsSub')} actions={<Chip tone="violet"><Icon name="cpu" className="w-3 h-3"/> {visibleRecs.length}</Chip>} bodyClass="overflow-y-auto p-2.5 space-y-2">
          <div style={{ maxHeight: 560 }} className="space-y-2">
            {visibleRecs.map((r) => <RecommendationCard key={r.id} rec={r} onAction={fire}/>)}
          </div>
        </Panel>
      </div>
