...
const Ico = ({ children }) => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" className="w-[28px] h-[24px]">
    {children}
  </svg>);
...
export default function KPICard({ label, value, unit, icon, color, rag: ragProp, trend, onClick, subValues }) {
    ...
    return (<div className="kpi-card" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
        <div className="rounded-xl flex items-center justify-center leading-none cwm-kpi-icon" style={{ width: '32px', height: '32px', fontSize: '1.15rem', flexShrink: 0 }}>
          {icon || '▣'}
        </div>
        <p className="font-semibold leading-tight flex-1" style={{ color: 'var(--app-text-muted)', fontSize: '12px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {label}
        </p>
        {hasTrend && (<span className="kpi-trend-badge font-semibold flex items-center leading-tight gap-0.5" style={{
                fontSize: '10px',
      ...
      <div style={{ flexShrink: 0, marginTop: '14px', display: 'flex', alignItems: 'baseline', gap: '4px' }}>
        <span className="font-bold leading-none tracking-tight" style={{ color: 'var(--app-text)', fontSize: 'clamp(2rem, 3.5vw, 2.8rem)' }}>
          {value}
        </span>
        {unit && (<span style={{ color: 'var(--app-text-faint)', fontSize: '0.85rem', fontWeight: 500 }}>{unit}</span>)}
      </div>

      {/* ── Row 3: sub-values ───────────────────────────────────────────── */}
      {subValues && subValues.length > 0 && (<div style={{ display: 'flex', gap: '20px', marginTop: '12px', flexShrink: 0, flexWrap: 'wrap' }}>
          {subValues.map((sv, i) => (<div key={i} style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        ...
        {onClick && (<button onClick={(e) => { e.stopPropagation(); onClick(); }} onKeyDown={(e) => e.key === 'Enter' && (e.stopPropagation(), onClick())} aria-label={`Details for ${label}`} title={`Details for ${label}`} style={{
