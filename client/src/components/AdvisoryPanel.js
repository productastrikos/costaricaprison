...
const toEvidenceStr = (e) => {
    ...
    if (e.message)
        return e.message;
    if (e.bin && (e.fill !== undefined))
        return e.bin + ': ' + e.fill + '% fill';
    if (e.vehicle && e.status)
        return e.vehicle + ': ' + e.status;
...
export default function AdvisoryPanel({ advisories = [], onClose }) {
    ...
    return (<div className="h-full flex flex-col relative overflow-hidden">
      ...
      {/* SUCCESS TOAST */}
      {successMsg && (<div className="absolute top-14 left-2 right-2 z-50 bg-emerald-500/20 border border-emerald-500/40 rounded-lg px-3 py-2 text-[10px] text-emerald-400 font-semibold shadow-xl">
          âœ“ {successMsg}
        </div>)}
      ...
      {actionForm && (<div className="absolute inset-x-0 bottom-0 z-40 bg-app-darker flex flex-col border-t border-app-border" style={{ top: 53 }}>
          {/* Form header */}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {ACTION_FORMS[actionForm.actionType]?.fields.map(field => (<div key={field.key}>
                <label className="block text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{field.label}</label>
                {field.type === 'select' ? (<select value={actionForm.formValues[field.key] || ''} onChange={e => setActionForm(f => ({ ...f, formValues: { ...f.formValues, [field.key]: e.target.value } }))} className={'w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-[11px] text-slate-200 focus:outline-none focus:ring-1 ' + formClr.ring}>
                    {field.opts.map(o => <option key={o} value={o} className="bg-slate-900 text-slate-200">{o}</option>)}
                  </select>) : field.type === 'textarea' ? (<textarea rows={3} value={actionForm.formValues[field.key] || ''} placeholder={field.placeholder} onChange={e => setActionForm(f => ({ ...f, formValues: { ...f.formValues, [field.key]: e.target.value } }))} className={'w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-[11px] text-slate-200 placeholder-slate-400 resize-none focus:outline-none focus:ring-1 ' + formClr.ring}/>) : (<input type="text" value={actionForm.formValues[field.key] || ''} placeholder={field.placeholder} onChange={e => setActionForm(f => ({ ...f, formValues: { ...f.formValues, [field.key]: e.target.value } }))} className={'w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-3 py-2 text-[11px] text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-1 ' + formClr.ring}/>)}
              </div>))}
          </div>

          {/* Submit area */}
          <div className="px-4 pb-4 pt-2 shrink-0 space-y-1.5 border-t border-app-border">
            <button onClick={submitForm} disabled={!!executing} className={'w-full py-2.5 text-[11px] font-bold rounded-lg border transition-colors disabled:opacity-50 ' + formClr.btn}>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2.5">
        {merged.map((advisory, idx) => {
            ...
            return (<div key={advisory.advisoryId} className={'advisory-card rounded-xl overflow-hidden transition-all duration-200 ' + pm.bg} style={{ background: '#d9d3ff' }}>

              {/* â”€â”€ Card header (always visible) â”€â”€ */}
              <button className="w-full text-left px-3.5 pt-3 pb-2.5 group" onClick={() => setExpandedId(isExpanded ? null : advisory.advisoryId)}>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <svg className={'w-3.5 h-3.5 text-white transition-transform group-hover:text-white/70 ' + (isExpanded ? 'rotate-180' : '')} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {isExpanded && (<div className="border-t-2 border-black/15 divide-y-2 divide-black/10">

                  {/* ROOT CAUSE */}
                  {advisory.rootCause && (<div className="px-3.5 py-3">
                      ...
                      <div className="pl-2 border-l-4 border-black/25 space-y-1.5">
                        {typeof advisory.rootCause === 'object' ? (<>
                            {advisory.rootCause.primary && (<div>
                            ...
                            {advisory.rootCause.contributing && (<div>
                            ...
                            {advisory.rootCause.systemic && (<div>
                  ...
                  {/* EVIDENCE */}
                  {advisory.evidence?.length > 0 && (<div className="px-3.5 py-3">
                      <p className="text-[10px] font-bold text-black uppercase tracking-widest mb-2">Evidence</p>
                      <div className="space-y-1">
                        {advisory.evidence.slice(0, 5).map((e, i) => (<div key={i} className="flex items-start space-x-2">
                  ...
                  {/* RECOMMENDATIONS */}
                  {recs.length > 0 && (<div className="px-3.5 py-3">
                      <p className="text-[10px] font-bold text-black uppercase tracking-widest mb-2">Recommendations</p>
                      <div className="space-y-1.5">
                        {recs.slice(0, 5).map((r, i) => (<div key={i} className="flex items-start space-x-2 pl-2 border-l-4 border-black/25">
                  ...
                  {impactEntries.length > 0 && (<div className="px-3.5 py-3">
                      ...
                      <div className="grid grid-cols-2 gap-1.5">
                        {impactEntries.map(([k, v]) => {
                            ...
                            return (<div key={k} className="bg-black/[0.06] border-2 border-black/15 rounded-lg px-2.5 py-2">
                  ...
                  {/* ACTION BUTTONS */}
                  {advisory.actions?.length > 0 && (<div className="px-3.5 py-3">
                      ...
                      <div className="space-y-1.5">
                        {advisory.actions.map(action => {
                            ...
                            return (<button key={aType} onClick={() => openForm(aType, advisory)} disabled={isExec} className={'w-full text-left px-3 py-2.5 rounded-lg border transition-all disabled:opacity-50 flex items-center justify-between group/btn ' + (aClr?.btn || COLOR_MAP.cyan.btn)}>
        ...
        {merged.length === 0 && (<div className="text-center py-12">
