...
export default function AlertPanel({ alerts = [], onClose }) {
    ...
    return (<div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {['critical', 'warning', 'info'].map(type => (grouped[type]?.length > 0 && (<div key={type}>
              <div className={`text-[10px] font-semibold uppercase tracking-wider mb-1.5 ${typeColors[type].text}`}>
                {type} ({grouped[type].length})
              </div>
              {grouped[type].slice(0, 15).map(alert => (<div key={alert.alertId} className={`${typeColors[type].bg} border ${typeColors[type].border} rounded-lg p-2.5 mb-1.5`}>
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-1.5">
                        ...
                        <button onClick={() => handleAcknowledge(alert.alertId)} className="text-[10px] text-cyan-400 hover:text-cyan-300 font-medium">
                          Acknowledge
                        </button>
                      </div>
                    </div>
        ...
        {activeAlerts.length === 0 && (<div className="text-center text-slate-400 text-sm py-8">
            No active alerts
          </div>)}
      </div>
    </div>);
