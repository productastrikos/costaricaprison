...
function getRag(value, thresholds, inverted, forceLevel) {
    ...
    if (!forceLevel) {
        if (thresholds) {
            ...
            if (!inverted)
                level = num >= green ? 'normal' : num >= amber ? 'warning' : 'critical';
            else
                level = num <= green ? 'normal' : num <= amber ? 'warning' : 'critical';
        }
        else {
    ...
    return {
        ...
        afterDraw(chart) {
            ...
            const draw = (val, color, text) => {
                if (val == null || isNaN(val) || val < y.min || val > y.max)
                    return;
                ctx.fillStyle = color;
...
function toCode(label) {
    ...
    if (words.length >= 2)
        return `${words[0].slice(0, 4).toUpperCase()}-${words[1].slice(0, 3).toUpperCase()}`;
...
export default function KPIDetailModal({ kpi, onClose, showAnalysis = true }) {
    ...
    const targetNum = useMemo(() => {
        if (kpi?.thresholds?.green != null)
            return kpi.thresholds.green;
     ...
     */
    const chartTargetVal = useMemo(() => kpi?.thresholds
        ? (kpi.inverted ? kpi.thresholds.amber : kpi.thresholds.green)
        : targetNum, [kpi, targetNum]);
    const chartWarnVal = useMemo(() => kpi?.thresholds ? (kpi.inverted ? kpi.thresholds.green : kpi.thresholds.amber) : null, [kpi]);
    const threshPlugin = useMemo(() => makePlugin(chartTargetVal, chartWarnVal, kpi?.inverted), [chartTargetVal, chartWarnVal, kpi?.inverted]);
    /* Threshold bands */
    const bands = useMemo(() => {
        const u = kpi?.unit || '';
        if (!kpi?.thresholds) {
    ...
    return (<div className="fixed inset-0 z-[1100] flex justify-end">
      ...
      <div className="relative h-full kpi-modal flex flex-col shadow-2xl animate-slide-in-right" style={{ width: 680, borderLeft: '1px solid var(--app-border)', borderRadius: 0, background: 'var(--app-panel)' }}>
        ...
        <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 8, padding: '10px 14px' }}>
        ...
        ].map((m, i) => (<div key={i} style={{ background: i === 0 ? rag.solidBg : tileBg, border: i === 0 ? `1px solid ${rag.solidBorder}` : tileBorder, borderRadius: 10, padding: '10px 14px' }}>
          ...
          <div style={{ flex: 1, minHeight: 0, display: 'flex', gap: 8 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div className="cwm-timeframe-control">
                    {availableRanges.map((r) => (<button key={r} onClick={() => { setTimeRange(r); setShowPrediction(false); }} className={`cwm-timeframe-btn ${activeTimeRange === r ? 'is-active' : ''}`}>{r}</button>))}
                  </div>
                  <button onClick={() => setShowPrediction((p) => !p)} className={`cwm-advisory-btn${showPrediction ? ' is-on' : ''}`} style={{ height: 24, padding: '0 8px', fontSize: 10, borderRadius: 5, boxShadow: showPrediction ? undefined : 'none' }}>
            ...
            <div style={{ flex: 2, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {/* Definition */}
              {kpi.definition && (<div style={{ flexShrink: 0, background: tileBg, border: tileBorder, borderRadius: 12, padding: '10px 12px' }}>
            ...
            return (<div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
              ...
              {subs.length > 0 && (<div style={{ flexShrink: 0, background: tileBg, border: tileBorder, borderRadius: 12, padding: '10px 12px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px 12px' }}>
                    {mainSubs.map((m, i) => (<div key={i} style={{ borderLeft: '2px solid #60a5fa', paddingLeft: 8 }}>
                    ...
                    {targetSubs.map((m, i) => (<div key={i} style={{ borderLeft: `2px solid ${innerDiv}`, paddingLeft: 8 }}>
          ...
          {showAnalysis && analyses.length > 0 && (<div style={{
                flexShrink: 0,
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0 14px' }}>
                {analyses.slice(0, 3).map((s, i) => (<div key={i} style={{ display: 'flex', gap: 9, alignItems: 'flex-start' }}>
                    <span style={{
